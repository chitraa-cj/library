...
- `server/routes.ts` defines `STRAPI_REPLACES_LOCAL = [{ strapiDocId, localPgSlug }, …]`. In `GET /api/books`, the legacy PG entry is matched **by slug** (not UUID) and filtered out only when its corresponding Strapi grantha is present in the same merged response — preserving DB-fallback behavior if Strapi is unreachable. Slug matching is required because the seeder generates a fresh `gen_random_uuid()` on every fresh database, so production's PG row has a different UUID from dev's, and a hard-coded UUID rule would silently fail to hide the duplicate in production.
